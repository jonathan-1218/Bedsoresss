...
export default function Analytics({ user, onLogout }) {
  ...
  const sourceBadgeClass = positionSource === "ml"
  ...
  return (
    <PageLayout user={user} onLogout={onLogout} title="Analytics" breadcrumb={["Analytics"]}>
      <div className="glass glow rounded-2xl p-5">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
          ...
          <div className="flex items-center flex-wrap gap-2">
            <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${isSensorOnline ? "bg-emerald-500/20 text-emerald-300 border-emerald-400/30" : "bg-rose-500/20 text-rose-300 border-rose-400/30"}`}>
              ESP32 {isSensorOnline ? "Online" : "Offline"}
            </span>
            <span className="text-slate-400 text-sm">Source</span>
            <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${sourceBadgeClass}`}>
              {positionSource}
            </span>
      ...
      <div className="glass glow rounded-2xl p-5">
        <h2 className="text-xl font-bold mb-1">Immobility Duration</h2>
